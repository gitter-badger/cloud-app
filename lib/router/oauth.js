...
module.exports = function (app, tfa, passport, login) {
  ...
  app.get('/api/v1/geofences', passport.authenticate('bearer', {session: false}), require('../../routes/api/v1/geofences').get);
  app.post('/api/v1/geofences', passport.authenticate('bearer', {session: false}), require('../../routes/api/v1/geofences').post);
